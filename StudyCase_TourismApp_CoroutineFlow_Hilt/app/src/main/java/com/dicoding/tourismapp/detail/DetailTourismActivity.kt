...
class DetailTourismActivity : AppCompatActivity() {
  ...
  private fun showDetailTourism(detailTourism: Tourism?) {
    detailTourism?.let {
      ...
      Glide.with(this@DetailTourismActivity).load(detailTourism.image).into(binding.ivDetailImage)

      var statusFavorite = detailTourism.isFavorite
      setStatusFavorite(statusFavorite)
      binding.fab.setOnClickListener {
        statusFavorite = !statusFavorite
        detailTourismViewModel.setFavoriteTourism(detailTourism, statusFavorite).run {
          if (this.hasObservers()) this.removeObservers(this@DetailTourismActivity)
          observe(this@DetailTourismActivity) {
            if (it) {
              setStatusFavorite(statusFavorite)
            }
          }
